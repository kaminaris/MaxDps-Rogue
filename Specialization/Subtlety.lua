...
function Subtlety:precombat()
    ...
    --if (MaxDps:CheckSpellUsable(classtable.SliceandDice, 'SliceandDice')) and cooldown[classtable.SliceandDice].ready then
...
function Subtlety:cds()
    ...
    if (MaxDps:CheckSpellUsable(classtable.Flagellation, 'Flagellation')) and (snd_condition and ruptures_before_flag and ComboPoints >= 6 and ttd >10 and ( cooldown[classtable.ShadowBlades].remains <= 2 or MaxDps:boss() and ttd <= 24 ) and ( not talents[classtable.InvigoratingShadowdust] or cooldown[classtable.SymbolsofDeath].remains <= 3 or buff[classtable.SymbolsofDeathBuff].remains >3 )) and cooldown[classtable.Flagellation].ready then
    ...
    if (MaxDps:CheckSpellUsable(classtable.ShadowDance, 'ShadowDance')) and (not buff[classtable.ShadowDanceBuff].up and MaxDps:boss() and ttd <= 8 + talents[classtable.Subterfuge]) and cooldown[classtable.ShadowDance].ready then
        MaxDps:GlowCooldown(classtable.ShadowDance, cooldown[classtable.ShadowDance].ready)
    end
    if (MaxDps:CheckSpellUsable(classtable.ShadowDance, 'ShadowDance')) and (not buff[classtable.ShadowDanceBuff].up and talents[classtable.InvigoratingShadowdust] and talents[classtable.DeathstalkersMark] and buff[classtable.ShadowBladesBuff].up and ( buff[classtable.SubterfugeBuff].up and targets >= 4 or buff[classtable.SubterfugeBuff].remains >= 3 )) and cooldown[classtable.ShadowDance].ready then
        MaxDps:GlowCooldown(classtable.ShadowDance, cooldown[classtable.ShadowDance].ready)
    end
    if (MaxDps:CheckSpellUsable(classtable.ShadowDance, 'ShadowDance')) and (not buff[classtable.ShadowDanceBuff].up and talents[classtable.UnseenBlade] and talents[classtable.InvigoratingShadowdust] and debuff[classtable.RuptureDeBuff].up and snd_condition and ( buff[classtable.SymbolsofDeathBuff].remains >= 6 and not buff[classtable.FlagellationBuffBuff].up or buff[classtable.SymbolsofDeathBuff].up and buff[classtable.ShadowBladesBuff].up or buff[classtable.ShadowBladesBuff].up and not talents[classtable.InvigoratingShadowdust] ) and ( cooldown[classtable.SecretTechnique].remains <10 + 12 * not talents[classtable.InvigoratingShadowdust] or buff[classtable.ShadowBladesBuff].up ) and ( not talents[classtable.TheFirstDance] or ( ComboPointsDeficit >= 7 and not buff[classtable.ShadowBladesBuff].up or buff[classtable.ShadowBladesBuff].up ) )) and cooldown[classtable.ShadowDance].ready then
        MaxDps:GlowCooldown(classtable.ShadowDance, cooldown[classtable.ShadowDance].ready)
    end
    ...
    if (MaxDps:CheckSpellUsable(classtable.ThistleTea, 'ThistleTea')) and (not buff[classtable.ThistleTeaBuff].up and ( buff[classtable.ShadowDanceBuff].remains >= 4 and buff[classtable.ShadowBladesBuff].up or buff[classtable.ShadowDanceBuff].remains >= 4 and cooldown[classtable.ColdBlood].remains <= 3 ) or ttd <= ( 6 * cooldown[classtable.ThistleTea].charges )) and cooldown[classtable.ThistleTea].ready then
