...
function Subtlety:callaction()
    ...
    stealth = buff[classtable.ShadowDanceBuff].up or buff[classtable.StealthBuff].up or buff[classtable.VanishBuff].up
    skip_rupture = buff[classtable.ShadowDanceBuff].up or buff[classtable.DarkestNightBuff].up or targets >= 8 and not talents[classtable.ReplicatingShadows] and talents[classtable.UnseenBlade]
    maintenance = ( debuff[classtable.RuptureDeBuff].up or skip_rupture ) and ( buff[classtable.SliceandDiceBuff].up or targets <= 2 )
    secret = buff[classtable.ShadowDanceBuff].up or ( cooldown[classtable.Flagellation].remains <40 and cooldown[classtable.Flagellation].remains >20 and talents[classtable.DeathPerception] )
    racial_sync = ( buff[classtable.ShadowBladesBuff].up and buff[classtable.ShadowDanceBuff].up ) or not talents[classtable.ShadowBlades] and buff[classtable.SymbolsofDeathBuff].up or MaxDps:boss() and ttd <20
    shd_cp = ComboPoints <= 1 or buff[classtable.DarkestNightBuff].up and ComboPoints >= 7 or calculateEffectiveComboPoints(ComboPoints) >= 6 and talents[classtable.UnseenBlade]
    Subtlety:cds()
    if (not stealth) then
        Subtlety:stealth_cds()
    end
    if (not buff[classtable.DarkestNightBuff].up and calculateEffectiveComboPoints(ComboPoints) >= 6 or buff[classtable.DarkestNightBuff].up and ComboPoints == ComboPointsMax) then
        Subtlety:finish()
    end
    Subtlety:build()
    if (not stealth) then
